...
class TrinoParamEscaper(common.ParamEscaper):
    def escape_datetime(self, item, format):
        _type = "timestamp" if isinstance(item, datetime.datetime) else "date"
        formatted = super(TrinoParamEscaper, self).escape_datetime(item, format, 3)
...
class Connection(object):
    """Trino does not have a notion of a persistent connection.
    ...
    def commit(self):
        """Trino does not support transactions"""
        pass
    ...
    def rollback(self):
        raise NotSupportedError("Trino does not have transactions")  # pragma: no cover
...
class Cursor(common.DBAPICursor):
    ...
    def __init__(self, host, port='8080', username=None, principal_username=None, catalog='hive',
        ...
        """
        :param host: hostname to connect to, e.g. ``Trino.example.com``
        ...
        :param poll_interval: float -- how often to ask the Trino REST interface for a progress
            update, defaults to a second
        :param source: string -- arbitrary identifier (shows up in the Trino monitoring page)
        ...
        :param KerberosRemoteServiceName: string -- Trino coordinator Kerberos service name.
        ...
        :param KerberosUseCanonicalHostname: boolean -- Use the canonical hostname of the
            Trino coordinator for the Kerberos service principal by first resolving the
        ...
        self._port = port
        """
        Trino User Impersonation: https://docs.starburstdata.com/latest/security/impersonation.html

        User impersonation allows the execution of queries in Trino based on principal_username
        argument, instead of executing the query as the account which authenticated against Trino.
        (Usually a service account)

        Allows for a service account to authenticate with Trino, and then leverage the
        principal_username as the user Trino will execute the query as. This is required by
        applications that leverage authentication methods like SAML, where the application has a
        username, but not a password to still leverage user specific Trino Resource Groups and
    ...
    def poll(self):
        """Poll for and return the raw status data provided by the Trino REST API.
    ...
    def _decode_binary(self, rows):
        # As of Trino 0.69, binary data is returned as the varbinary type in base64 format
